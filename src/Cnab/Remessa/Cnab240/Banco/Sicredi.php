...
class Sicredi extends AbstractRemessa implements RemessaContract
{
    ...
    protected function headerLote()
    {
        ...
        $this->add(1, 3, '748'); //Codigo do banco
    ...
    {
        $this->iniciaDetalhe();
        $this->add(1, 3, '748'); //Código do Banco
        ...
        $this->add(23, 23, ''); // Digito verificador da Agência do cedente
        ...
        $this->add(106, 106, '');
        $this->add(107, 108, '03'); //Espécie do título 03=DM
        $this->add(109, 109, 'N'); //Identif. de título Aceito/Não Aceito
        ...
        $this->add(224, 224, '1'); //Código para Baixa/Devolução
        $this->add(225, 227, '060');
    ...
    public function segmentoQ($nSequencialLote, BoletoContract $boleto)
    {
        ...
        $this->add(8, 8, '3'); // Numero do lote remessa
        $this->add(9, 13, Util::formatCnab(9, $nSequencialLote, 5)); // Nº sequencial do registro de lote
        $this->add(14, 14, 'Q'); // Nº sequencial do registro de lote
        $this->add(15, 15, ''); // Reservado (Uso Banco)
        $this->add(16, 17, '01'); // Código de movimento remessa
        ...
        $this->add(154, 154, '0'); // Tipo de inscrição do avalista
        $this->add(155, 169, ''); // Tipo de inscrição do avalista
        $this->add(170, 209, ''); // Nome do avalista
    ...
    public function segmentoR($nSequencialLote, BoletoContract $boleto)
    {
        ...
        $this->add(8, 8, '3'); // Numero do lote remessa
        $this->add(9, 13, Util::formatCnab(9, $nSequencialLote, 5)); // Nº sequencial do registro de lote
        $this->add(14, 14, 'R'); // Nº sequencial do registro de lote
        $this->add(15, 15, ''); // Reservado (Uso Banco)
        $this->add(16, 17, '01'); // Código de movimento remessa
        $this->add(18, 65, Util::formatCnab(9, 0, 48)); //varias coisas que manda tudo 0
        $this->add(66, 66, '2');
        $this->add(67, 74, Util::formatCnab(9, $sDataCobrancaEncargos, 8));
        $this->add(75, 89, Util::formatCnab(9, $boleto->getMulta(), 15, 2));
        $this->add(90, 199, ''); //varios espacinhos
        $this->add(200, 207, '00000000');
        $this->add(208, 210, '748');
        $this->add(211, 215, Util::formatCnab('9', $this->getAgencia(), 5));

        $ageDV = $this->getAgenciaDv();
        if(strlen($ageDV) > 1){
            $ageDV = '0';
        }
    ...
    protected function trailer()
    {
        ...
        $this->add(9, 17, ''); // Reservado (Uso Banco)
        $this->add(18, 23, '000001'); // Qtd de lotes do arquivo
